...
export const createCollection = async (req, res) => {
    try {
        ...
        if (user.users.find(u => u.userId.toString() === userId.toString()).privilege < admin_grade) return res.status(403).json({ message: "User not authorized" });
...
export const getAllRequestsFromCollection = async (req, res) => {
    try {
        ...
        return res.status(200).json(collection.requests);
    } catch (error) {
        return res.status(500).json({ message: error.message });
    }
}
...
export const changeCollectionName = async (req, res) => {
    try {
        ...
        return res.status(200).json({message: "Collection name updated successfully"});
    } catch (error) {
        return res.status(500).json({ message: error.message });
    }
}
